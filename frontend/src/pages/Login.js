...
const Login = () => {
  ...
  const handleSeedDatabase = async () => {
    try {
      ...
      alert(`Database seeded!\nAdmin Email: ${response.data.admin_email}\nPassword: ${response.data.admin_password}`);
    } catch (err) {
      if (err.response?.data?.message === 'Database already seeded') {
        alert('Database already seeded. Use admin@warehouse.com / admin123');
      } else {
        alert('Seeding error. Try admin@warehouse.com / admin123');
      }
    }
  };
    ...
    >
      <Card className="w-full max-w-md shadow-2xl border-0">
        <CardHeader className="text-center pb-2">
          ...
          <CardTitle className="text-2xl font-bold text-gray-900">WMS Pro</CardTitle>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="text-center space-y-2">
            <p className="text-sm text-gray-500">
              ...
              <Link to="/register" className="text-[#f59e0b] hover:text-[#d97706] font-medium" data-testid="register-link">
                Register
              </Link>
            </p>
            <button
